...
def plot_magnification(x, y, magn, res, origin, solver, xmin=None, xmax=None,
	...
	plt.scatter(x, y, c=magn, s=6, marker = 'o', cmap='gray', lw=None)
...
def abbreviate(origin, solver):
	...
	if origin == 'geo_cent':
		origin_str = 'gcent'
		origin_title = 'Geo Center'
	elif origin == 'plan':
		origin_str = origin
		origin_title = 'Planet'
	elif origin == 'com':
		origin_str = origin
		origin_title = 'Center Mass'
	elif origin == 'star':
		origin_str = origin
		origin_title = 'Star'
	...
	if solver == 'numpy':
		...
		solver_str = 'SG'
		solver_title = 'SG 2012'
	else:
		 solver_str = solver
	return origin_str, origin_title, solver_str, solver_title
...
for (i, hdul) in enumerate(hdul):
	...
	xmax=x[0][-1]
	xcent = (xmax + xmin) / 2.
	xmin = xcent - 0.4 * (xmax - xmin)
	xmax = xcent + 0.8 * (xmax - xcent)
	ymin=y[0][0]
	ymax=y[0][-1]
	plot_magnification(x=x_new, y=y_new, magn=magn_new, res=0.2*res[i], origin=origin[i],
							solver=solver[i], xmin=xmin, xmax=xmax, ymin=0.8*ymin, ymax=0.8*ymax)
	(origin_str, origin_title, solver_str, solver_title) = abbreviate(
											origin=origin[i], solver=solver[i])
	plt.gcf().set_size_inches(8, 6)
	plt.title('High Magnification with Caustic\nFrame: {}; Solver = {}; M > {}'.format
					(origin_title, solver_title, cutoff))
	plt.savefig('../Tables/high_mag_{}_{}'.format(solver_str, origin_str))
	plt.show()
